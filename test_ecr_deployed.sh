...
# Test deployed functionality using ECR image locally
# This simulates the deployed environment using the ECR image
...
check_prerequisites() {
    ...
    # Check if ECR image exists locally, if not pull it
    if ! docker images | grep -q "669655810547.dkr.ecr.us-east-2.amazonaws.com/multimodal-transcription"; then
...
# Function to run ECR image test
run_ecr_test() {
    print_info "Running deployed functionality test with ECR image..."
    
    # Create output directory in S3
    aws s3api put-object --bucket $S3_BUCKET --key "$S3_OUTPUT_PREFIX/test-started.txt" --body /dev/null
    
    # Run the ECR container with S3 configuration
    print_info "Starting ECR container with S3 video and output..."
    ...
    if [ $? -eq 0 ]; then
        print_success "ECR test completed successfully!"
    else
        print_error "ECR test failed!"
        return 1
    fi
}
...
# Main execution
print_info "Testing Deployed Functionality with ECR Image"
print_info "=============================================="
